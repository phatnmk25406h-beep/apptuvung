...
# --- HÀM 1: Đánh giá ---
def danhgia(D, T):
    # D = Số lần đúng, T = Tổng số lần đoán
    ti_le_dung= D/T
...
def bat_dau_hoc():
    try:
        ...
        # 4. Hiển thị từ đầu tiên
        hien_tu_moi()
    except FileNotFoundError:
        form.lnetienganh.setText("LỖI: Không tìm thấy file, hãy copy file ở trên")
...
# (Chúng ta dùng nó làm nút "Kiểm tra" và nó cũng là nút hiện từ tiep theo)
def kiem_tra_dap_an():
    try:
        ...
        if b == form.randomlist[1]: # randomlist[1] chính là từ tiếng việt ứng voi từ tiếng anh đó
            # Nếu ĐÚNG, xóa từ này khỏi danh sách
            form.list_of_list.remove(form.randomlist)
            #tăng số lần đúng
            form.D = form.D + 1
        else:
            form.txtloikhuyen.setText(f"Đáp án đúng là {form.randomlist[1]} ")
        # 3. Cập nhật giao diện điểm số
        form.txtlannhapsai_2.setText(f"{form.D} / {form.T}")
        # 4. Hiển thị từ tiếp theo
        hien_tu_moi()
    except Exception as e:
